...
class station_zdz:
    def __init__(self):
        ...
        self.station_taizhou = ['K8705','58559','K8706','K6125','K6122','K8910','58652','K8903','58653','58568','K8818',
                                '58662','K8821','K8609','58660','58663','K8611','58661','58665','K8301','K8201',
                                'K8217','K8282','58666','K8413','58664','58669','K8505','58667','58668','K8218']
            ...
            if value_index!=0: 
                ...
                if  value_index ==1 or value_index ==2:     
                    if (boundary[3]-boundary[2])<2.9:
                        ...
                        remain = boundary_data[(boundary_data['Station_Id_C'].isin(self.station_taizhou))|(boundary_data['Station_levl']==11)| (boundary_data['Station_levl']==15)| (boundary_data['Station_levl']==12)| (boundary_data['Station_levl']==13)| (boundary_data['Station_levl']==16)]
                    else:
                        remain = boundary_data[(boundary_data['Station_Id_C'].isin(self.station_taizhou))|(boundary_data['Station_levl']==11)| (boundary_data['Station_levl']==12)| (boundary_data['Station_levl']==13)| (boundary_data['Station_levl']==16)]
                # 风力 --- 显示8及以上大风
                elif value_index ==3:
                    if (boundary[3]-boundary[2])<2.9:
                        remain = boundary_data
...
class station_text:
    ...
    def text_wind(self,plot_data):
        ...
        rank_station = {
            ...
            '6级风以下':len(wind[(wind['wind']>0) & (wind['wind']<10.7)].value_counts())
        }
        #  统计各等级风力的个数
        numbertext = ""
    ...
    def text_view(self,plot_data):
        ...
        rank_station = {
           '强浓雾':len(view[(view['view']>0) & (view['view']<50)].value_counts()),
    ...
    def text_tmax(self,plot_data):
        ...
        rank_station = {
           ...
           '正常':len(tmax[(tmax['tmax']>-40) & (tmax['tmax']<35)].value_counts())      
        }
        # 获取单站较大的有
        ...
        if rank_station['40度以上'] >0 :
            ...
            text = "【高温通报】 全市出现37℃以上的高温，温度较高的有："
            text = text + indextext + "其中，"  + numbertext  
        elif rank_station['35度以上'] >0 :
            text = "【高温通报】 全市出现35℃以上的高温，温度较高的有："
            text = text + indextext 
        else:
            text = ""
    ...
    def text_rain(self,plot_data):
        ...
        for key, value in rank_station_town.items():
            if value>0 and key not in ["≥0毫米"]:
                numbertext_town = numbertext_town + key +"的有" + str(value) + "个乡镇,"
...
class data2sql():
    ...
    def to_redis(self,data):
        ...
        rs.set("warring_zdz", pickle.dumps(data))
    def get_data(self):
...
class FTP_Radar(object):
    ...
    def to_redis(self,imd):
        ...
        rs = redis.Redis(host='10.137.13.151', port=16379,password="tzqxj58660")
        rs.set("radar", pickle.dumps(data)) 
    def get_radar(self):
...
#         f = cinrad.io.StandardData(path+'Z_RADR_I_Z9576_20230702105550_O_DOR_SAD_CAP_FMT.bin.bz2')
    ...
    def to_vcs(self,rl):
        ...
        rs = redis.Redis(host='10.137.13.151', port=16379,password="tzqxj58660")
        rs.set("radar_vsc", pickle.dumps(data)) 
    def get_vcs(self):
...
class server_plot():
    ...
    def color_map(self):
        if self.plot_type=="rain":
            ...
            if hours >12:
                ...
                cmap_nonlin = mpl.colors.ListedColormap(colorslist)  # 自定义颜色映射 color-map
                norm = mpl.colors.BoundaryNorm(levels, cmap_nonlin.N)  # 基于离散区间生成颜色映射索引 
        return cmap_nonlin,levels,norm
    ...
    def plot_rain(self):
        ...
        cmaps,levels,norm = self.color_map()
        lat = data_xr.lat
        lon = data_xr.lon
        lons, lats = np.meshgrid(lon, lat)
        contour = plt.contourf(lons,lats,data_xr,cmap=cmaps,levels=levels,norm = norm)
        geojson = geojsoncontour.contourf_to_geojson(
            contourf=contour,
            ndigits=3,
            unit='mm'
        ...
        if self.time_hours ==0.1:   
            ...
            if raintop['PRE'].values[0]>0.5: 
                ...
                if len(rain_tz[rain_tz['rank']==1])>0:
                    ...
                    if len(rain_tz[rain_tz['rank']==1])>0:
                        text_rain = text_rain + "其中超过5毫米的站有" + str(len(rain_tz[rain_tz['rank']==1])) + "个；"          
            else:
                text_rain = ""
    ...
    def return_province(self):
        ...
        text_cnty_average = text_cnty_average[:-1] + "。"
        text_rain = text_rain + text_cnty_average
        wind = wind[wind['value']>17.2].sort_values(by="value",ascending=False)
        text = ""
        wind_text = ""
        if len(wind)>0:
...
class PredictGrid(object):
    def __init__(self):
        ...
        :param port: ftp端口 （默认21）
        """
        self.host = "10.135.30.179"
        self.username = "tiga"
        self.password = "tiga1234"
        self.port = 21
    def ftp_connect(self):
        """
        连接ftp
    ...
    def download_file(self):
        ...
        :return:
        """
        dst_file_path = "/home/workspace/Data/Igrad"
        download_list = []
        now = dtt.datetime.now()
        offset = dtt.timedelta(days=0)
        today = (now + offset)
        ftp_file_path = "/realtimedata/nafp/IGrid/Integers/Latest/"+today.strftime('%Y.%m') + "/"+today.strftime('%Y%m%d')+"/"
        local_file_path = today.strftime('%Y.%m') + "/" +today.strftime('%Y%m%d') + "/"
        buffer_size = 102400 #默认是8192
        ftp = self.ftp_connect()
        file_list = ftp.nlst(ftp_file_path)
        #print(file_list) 
        if os.path.isdir(dst_file_path+"/" + local_file_path):
            ...
            local_files = os.listdir(dst_file_path+"/" +  local_file_path)
            for download in file_list:
                if download[58:] not in local_files:
                    download_list.append(download[58:])                
        else:
            #print("文件夹不存在就创建")
            os.makedirs(dst_file_path+"/" + local_file_path) 
        for file_name in download_list: 
    ...
    def del_file(self):
        today = dtt.datetime.utcnow()
        offset = dtt.timedelta(days=-15)
        delday = (today + offset).strftime('%Y.%m') + "/" + (today + offset).strftime('%Y%m%d')
        path = "/home/workspace/Data/Igrad/" + delday
        if os.path.isdir(path): 
            shutil.rmtree(path)
    def color_map(self,hours):
        if hours >12:
            colorslist = ['#FFFFFF','#A3FAFD', '#29D3FD', '#29AAFF', '#2983FF', '#4EAB37', '#46FA35', '#F1F837', '#F1D139', '#F2A932', '#F13237', '#C4343A', '#A43237', '#A632B4', '#D032E1', '#E431FF']# 24降水
            levels = [-1,0.01, 5, 10, 15, 20, 25, 35, 50, 75, 100, 150, 200, 250, 350, 500]
            cmap_nonlin = mpl.colors.ListedColormap(colorslist)  # 自定义颜色映射 color-map
    ...
    def get_file(self):
        now = dtt.datetime.now()
        offset = dtt.timedelta(days=0)
        today = (now + dtt.timedelta(days=0))
        yestday = (now + dtt.timedelta(days=-1))
        if now.hour>=11 and now.hour<=16:
            filepath = "/home/workspace/Data/Igrad/"+today.strftime('%Y.%m') + "/"+today.strftime('%Y%m%d')+"/"
            filelist = fnmatch.filter(os.listdir(filepath),"GFE.P330000*T1000*Pr01.nc")
        elif now.hour>16 and now.hour<=23:
            filepath = "/home/workspace/Data/Igrad/"+today.strftime('%Y.%m') + "/"+today.strftime('%Y%m%d')+"/"
            filelist = fnmatch.filter(os.listdir(filepath),"GFE.P330000*T1615*Pr01.nc")
        elif now.hour>=7 and now.hour<11:
            filepath = "/home/workspace/Data/Igrad/"+today.strftime('%Y.%m') + "/"+today.strftime('%Y%m%d')+"/"
            filelist = fnmatch.filter(os.listdir(filepath),"GFE.P330000*T0545*Pr01.nc")
        else:
            filepath = "/home/workspace/Data/Igrad/"+yestday.strftime('%Y.%m') + "/"+yestday.strftime('%Y%m%d')+"/"
            filelist = fnmatch.filter(os.listdir(filepath),"GFE.P330000*T1615*Pr01.nc")
        data = xr.open_dataset(filepath+filelist[-1])
        return data
    def return_label(self,date):
        now = int(date/1000)
        timeArray = time.localtime(now)
        otherStyleTime = time.strftime("%Y-%m-%d %H:%M", timeArray)
        return otherStyleTime
    def plot_rain(self,start,end):
        start = start[0:13] + ":00:00"
        end = end[0:13] + ":00:00"
        data = self.get_file()
        startt = dtt.datetime.strptime(start,'%Y-%m-%d %H:%M:%S')
        endt = dtt.datetime.strptime(end,'%Y-%m-%d %H:%M:%S')
        hours = (endt-startt).total_seconds() / 3600
        cmaps,levels,norm= self.color_map(hours)
        data_xr = data.Pr01.loc[start:end].sum(dim=['time'])
        time = data.time.loc[:]
        starttime = self.return_label(time.values.tolist()[0]/1000000)
        endtime = self.return_label(time.values.tolist()[-1]/1000000)
        if startt>dtt.datetime.strptime(starttime,'%Y-%m-%d %H:%M'):
            starttime = start[0:13] +":00"
        if endt<dtt.datetime.strptime(endtime,'%Y-%m-%d %H:%M'):
            endtime = end[0:13] +":00" 
        label = starttime + " 至 " + endtime + " 累积降水"  
        lat = data_xr.lat
        lon = data_xr.lon
        lons, lats = np.meshgrid(lon, lat)
        contour = plt.contourf(lons,lats,data_xr,cmap=cmaps,levels =levels, norm=norm)
...
class message:
    def __init__(self):
        self.rs = redis.Redis(host='127.0.0.1', port=6379)
        self.conn = pymysql.connect(host="127.0.0.1",port=3306,user="root",passwd="tzqxj58660",db="ZJSZDZDB")
        self.server = "172.21.158.221"    # 连接服务器地址
        self.user = "xlyrw"# 连接帐号
        self.password = "xlyrw1"# 连接密码
    def update_server_time(self):
        ntp_client = ntplib.NTPClient()
        response = ntp_client.request('10.135.2.24')
        timestamp = response.tx_time
        offset = response.offset
        local_time = time.localtime(timestamp - offset)
        times = time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(timestamp))
        # 执行语句
        command = """sudo -S date -s '{times}'"""
    ...
    def check_status(self):
        ...
        now = dtt.datetime.utcnow()
        end = (now+dtt.timedelta(minutes=0)).strftime('%Y-%m-%d %H:%M')+":00"
        start = (now+dtt.timedelta(minutes=-3*60)).strftime('%Y-%m-%d %H:%M')+":00"
        job = "table_station_min"
        conn = pymysql.connect(host="127.0.0.1",port=3306,user="root",passwd="tzqxj58660",db="tzweb")
        rsql = """select * from django_apscheduler_djangojobexecution where run_time > '{start}' and job_id='{job}' """
        sql = rsql.format(start=start,job=job)
    ...
    def send_msg(self):
        ...
        a = data[(data['status']!='Executed')] 
        b = a[a['status']!='Started execution']
        if len(b)>3:
            status = b['status'].values[0]
            rsql = """insert into AssistantInfo.dbo.短信息 (tTime,SendMsg,TelNo,Sender,City,Country,Status) values('{tTime}','{msg}','{num}','{tit1}','{tit2}','{tit3}','{sendid}') """
            tTime = str(end)
            num = str(13615762563)
            errtime = b['run_time'].values[0]
            msg =  "错误提醒:" + str(errtime)[:]+ ":" + status
            tit1 = "台州市\市本级"
            tit2 = "台州市"
            tit3 = "市本级"
            sendid = 0
            sql = rsql.format(tTime =tTime,num = num,msg = msg,tit1 = tit1,tit2 = tit2,tit3=tit3,sendid = sendid )
            conn = pymssql.connect(self.server, self.user, self.password, "AssistantInfo") 
            cursor = conn.cursor()
            cursor.execute(sql)
            cursor.close()
            cursor = conn.cursor()
...
class station_days:
    def __init__(self):
        ...
        self.dataFormat = "json"
    # 以下为数据同步的方法
    def get_datelabel(self):
        now = dtt.datetime.utcnow()
        end = (now+dtt.timedelta(hours=-8)).strftime('%Y%m%d')+"000000"
        start = (now+dtt.timedelta(hours=-(24+8))).strftime('%Y%m%d')+"000000"
        label = "[" + start + "," + end +"]"
        return label
    def get_preday(self):
        '''run pre hours'''
        eles = "Cnty,Province,Town,Station_levl,Station_Name,City,Station_Id_C,Lat,Lon,Alti,Datetime,TEM_Max,TEM_Min,VAP_Avg,RHU_Avg,RHU_Min,VIS_Min,PRE_Max_1h,PRE_Time_2008,PRE_Time_0820,PRE_Time_2020,PRE_Time_0808,WIN_D,WIN_S,WIN_S_Max,WIN_S_Max_OTime,WIN_D_INST_Max,WIN_S_Inst_Max,WIN_S_INST_Max_OTime,GST_Max,GST_Min,SSH"  
        
        label = self.get_datelabel()
        client = DataQueryClient(configFile=r"/home/workspace/Data/My_Git/web_met/bak/zdz/common/utils/client.config")
    ...
    def data2mysql(self):
        ...
        conn = pymysql.connect(host="127.0.0.1",port=3306,user="root",passwd="tzqxj58660",db="ZJSZDZDB")
        cursor = conn.cursor()
        insql = """insert ignore into station_nation_day 
        (Cnty,Province,Town,Station_levl,Station_Name,
        City,Station_Id_C,Lat,Lon,Alti,Datetime,TEM_Max,
        TEM_Min,VAP_Avg,RHU_Avg,RHU_Min,VIS_Min,PRE_Max_1h,
        PRE_Time_2008,PRE_Time_0820,PRE_Time_2020,PRE_Time_0808,
        WIN_D,WIN_S,WIN_S_Max,WIN_S_Max_OTime,WIN_D_INST_Max,
        WIN_S_Inst_Max,WIN_S_INST_Max_OTime,GST_Max,GST_Min,SSH) 
